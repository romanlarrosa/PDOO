...
module ModeloQytetet
  class Qytetet
    ...
    attr_accessor:mazo,:tablero, :cartaActual, :dado, :jugadorActual, :jugadores, :EstadoJuego
    ...
    def cancelarHipoteca(numCasilla)
      titulo = @tablero.obtenerCasillaNumero(numCasilla)
    ...
    def edificarCasa(numcasilla)
      ...
      if(edificada)
         @EstadoJuego = EstadoJuego::JA_PUEDE_GESTIONAR
      end
      return edificada
    end
    ...
    def obtenerPropiedadesJugadorSegunEstadoHipoteca(estadoH)
      numeros = Array.new
      for i in obtenerPropiedadesJugador()
        if(@tablero.casillas[i].titulo.hipotecada)
          numeros << @tablero.casillas[i].titulo
        end
      end
      return numeros
    end
